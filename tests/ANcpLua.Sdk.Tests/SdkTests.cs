    ...
    SdkImportStyle sdkImportStyle)
{
    ...
    {
        await using var project = CreateProjectBuilder();
        project.AddCsprojFile(properties: [("IsTestProject", "true")]);
        var data = await project.BuildAndGetOutput();
    ...
    public async Task MTP_DotnetTestSkipAnalyzers()
    {
        ...
        project.AddCsprojFile(
            filename: "Sample.Tests.csproj",
            properties: [("IsTestProject", "true"), ("UseMicrosoftTestingPlatform", "true")],
    ...
    public async Task MTP_SuccessTests(bool addUseMicrosoftTestingPlatformProperty)
    {
        ...
        project.AddCsprojFile(
            filename: "Sample.Tests.csproj",
            properties: addUseMicrosoftTestingPlatformProperty
                ? [("IsTestProject", "true"), ("UseMicrosoftTestingPlatform", "true")]
                : [("IsTestProject", "true")],
            nuGetPackages: [.. XUnit3MTP2References]
        );
    ...
    {
        await using var project = CreateProjectBuilder();
        project.AddCsprojFile(rootSdk: "Microsoft.NET.Sdk.Web", importStyle: SdkImportStyle.SdkElement);

        project.AddFile("Program.cs", """
                                      using ANcpLua.AspNetCore.ServiceDefaults;

                                      var builder = WebApplication.CreateBuilder();
                                      builder.UseANcpLuaConventions();
                                      """);
    ...
    public async Task Web_ServiceDefaultsIsRegisteredAutomatically()
    {
        ...
        project.AddFile("Program.cs", """
                                      ...
                                      return app.Services.GetService<ANcpLuaServiceDefaultsOptions>() is not null ? 0 : 1;
                                      """);
    ...
    public async Task Web_ServiceDefaultsIsRegisteredAutomatically_Disabled()
    {
        ...
        project.AddCsprojFile(
            ...
            importStyle: SdkImportStyle.SdkElement,
