# core/ — TypeSpec & Generated Code

@../.claude/instructions/architecture.md

## Scope

This directory contains the single source of truth for all data models. TypeSpec definitions generate OpenAPI which generates Kiota clients.

## Execution

<typespec_modification>
When modifying TypeSpec schemas, follow these steps in order (you will be penalized if you skip steps):

1. Edit files in `core/specs/` only
2. Run `nuke TypeSpecCompile` - verify no TypeSpec errors
3. Run `nuke GenerateAll` - regenerate all clients
4. Verify output in `core/generated/dotnet/Models/`
5. Fix any consumer compilation errors

You ***must not*** edit files in `core/generated/` directly. Changes will be overwritten.
</typespec_modification>

<file_ownership>
| Directory | Editable | Purpose |
|-----------|----------|---------|
| `specs/` | YES | TypeSpec source files |
| `openapi/` | NO | Generated OpenAPI 3.1 |
| `generated/dotnet/` | NO | Generated C# (Kiota) |
| `generated/typescript/` | NO | Generated TypeScript |
| `generated/python/` | NO | Generated Python |
</file_ownership>

<typespec_conventions>
```typescript
// Use @encodedName for wire format
model SpanData {
  traceId: string;

  @encodedName("json", "gen_ai.provider.name")
  genAiProviderName?: string;

  @encodedName("json", "gen_ai.usage.input_tokens")
  genAiInputTokens?: int64;
}
```

Wire format: snake_case with dots (`gen_ai.provider.name`)
C# property: PascalCase (`GenAiProviderName`)
</typespec_conventions>

## Commands

```bash
nuke TypeSpecCompile     # specs → openapi
nuke GenerateCSharp      # → dotnet/
nuke GenerateTypeScript  # → typescript/
nuke GenerateAll         # All targets
```
