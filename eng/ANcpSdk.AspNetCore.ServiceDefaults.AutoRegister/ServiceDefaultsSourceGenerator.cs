...
public sealed class ServiceDefaultsSourceGenerator : IIncrementalGenerator
{
    ...
    {
        var hasType = context.CompilationProvider.Select((compilation, cancellationToken) =>
            compilation.GetTypeByMetadataName("ANcpSdk.AspNetCore.ServiceDefaults.ANcpSdkServiceDefaults") is not null);

        var syntax = context.SyntaxProvider
            .CreateSyntaxProvider(
                (syntaxNode, cancellationToken) => syntaxNode.IsKind(SyntaxKind.InvocationExpression),
                Transform)
        ...
        static InterceptionData? Transform(GeneratorSyntaxContext context, CancellationToken cancellationToken)
        {
            ...
            if (invocation.TargetMethod.Name is "Build" &&
            ...
            static string CreateOrderKey(SyntaxNode node)
            {
                ...
                return node.SyntaxTree.FilePath + ":" + lineSpan.StartLinePosition.Line + ":" +
